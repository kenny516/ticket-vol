...
<% List<Vol> vols = (List<Vol>) request.getAttribute("vols");
    ...
    Integer villeDepartId = (request.getAttribute("villeDepartId") !=
            null) ?
            (Integer) request.getAttribute("villeDepartId") : null;
    Integer villeArriveId = (request.getAttribute("villeArriveId") !=
            null) ?
            (Integer) request.getAttribute("villeArriveId") : null;
    String dateDepart = (request.getAttribute("dateDepart") != null) ?
            (String)
                    request.getAttribute("dateDepart") : "";

    Double maxPrice = (request.getAttribute("maxPrice") != null) ?
            (Double)
...
<div class="container mt-4">
    <div class="row mb-4">
        <div class="col">
            <div class="card">
                <div class="card-body">
                    <h2 class="card-title">Recherche de Vols
                    </h2>
                </div>
            </div>
        </div>
        <div class="col text-end">
            <% Utilisateur user = (Utilisateur)
                ...
                if (user != null) {
            %>
            <span class="me-3">Bienvenue, <%= user.getNom()
            %></span>
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form
                    action="<%= request.getContextPath() %>/vols/search"
                ...
                <div class="col-md-3">
                    ...
                    <input type="date" name="dateDepart"
                           id="dateDepart" class="form-control"
                           value="<%=dateDepart %>">
                </div>

                <div class="col-md-2">
                    <label for="maxPrice"
                           class="form-label">Prix maximum</label>
                ...
                <div class="col-md-1">
                    ...
                    <button type="submit"
                            class="btn btn-primary w-100">Filtrer
                    </button>
                </div>
            </form>
        </div>
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tr>
                        ...
                        <td>
                            <% for (PlaceVol placeVol :
                                    vol.getPlaceVols()) {
                                String
                                        designation = placeVol.getTypeSiege().getDesignation();
                                double
                                        prix = placeVol.getPrix();
                            %>
                            <p>
                                <%= designation %> -
                                <%= prix %>AR
